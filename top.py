        ...
        **kwargs,
        ...
        if enable_control:  # Add supporting hardware for the cpu
            ...
            self.submodules.spiflash = SpiFlash(
                platform.request("spiflash4x"), dummy=6, endianness="little"
            )
            self.register_mem(
                "spiflash",
            ...
            self.add_memory_region(
                "rom",
                self.mem_map["spiflash"] + bios_flash_offset,
                spiflash_size - bios_flash_offset,
                type="cached+linker",
            )
        ...
        if enable_dip:
            ...
            self.submodules.dip = DipSwitchPmod(pads=platform.request_all("dip_switch"))

        if enable_7seg:
            # 7 Segment
            platform.add_extension(platformExtensions.seven_segment_pmod_1a)
            self.submodules.seven_segment = SevenSegmentPmod(
                pads=platform.request_all("seven_segment"), sys_clk_freq=sys_clk_freq
            )
...
def main():
    ...
    parser.add_argument(
        "--build", action="store_true", default=True, help="Build bitstream"
    )
    parser.add_argument(
        "--load", action="store_true", default=False, help="Load bitstream"
    )
    parser.add_argument(
        "--flash", action="store_true", default=True, help="Flash Bitstream"
    )
    parser.add_argument(
        "--sys-clk-freq", default=21e6, help="System clock frequency (default: 24MHz)"
    )
    parser.add_argument(
        "--bios-flash-offset",
        default=0x40000,
    ...
    parser.add_argument(
        "--sevenseg", action="store_true", help="Enable 7 segment LED block"
    )
    parser.add_argument(
        "--control",
        action="store_true",
        help="Enable control block (will enable --dip and --7seg, and instantiates a cpu)",
    )
    builder_args(parser)
    ...
    soc = BaseSoC(
        ...
        blink_leds=args.blinky,
        enable_dip=args.dip,
        enable_7seg=args.sevenseg,
